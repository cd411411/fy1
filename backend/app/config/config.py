...
class Settings(BaseSettings):
    ...
    DATABASE_URL: str = "postgresql+asyncpg://myuser:mypassword@db:5432/mydatabase"
    ...
    REDIS_PORT: int = 6379
    VECTOR_STORE_PROVIDER: Literal["chroma", "qdrant"] = "qdrant"
    ...
    def SYNC_DATABASE_URL(self) -> str:
        """
        为 Alembic 自动生成一个同步的数据库URL。
        它通过移除异步驱动部分 (+asyncpg) 来实现。
        """
        # Alembic 在某些配置下需要一个不带异步驱动标识的URL

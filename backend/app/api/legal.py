...
async def analyze_regulations_structured(request: LegalAnalysisRequest,
    ...
    try:
        response_json = await get_ai_json_response(prompt,db,request_source="法律适用",use_rag=True,case_cause=request.case_cause,user_query=request.content)
...
async def analyze_case(request: CaseAnalysisRequest,
    ...
    try:
        response_json = await get_ai_json_response(prompt, db,request_source="案件预判")
...
async def analyze_objection_points(request: ObjectionAnalysisRequest,
    ...
    **你必须严格按照以下JSON格式返回，不要有任何额外解释或Markdown标记。**
    ...
    """
    try:
        response_json = await get_ai_json_response(prompt, db,request_source="异议建议")
        # Pydantic会自动验证新的结构
...
async def adjudicate_document(request: DocumentAdjudicationRequest,
    ...
    **输出格式 (必须严格遵守，不要添加任何额外解释):**
    ...
    try:
        response_json = await get_ai_json_response(prompt, db,request_source="文书内容审核")
    ...
    try:
        if isinstance(extracted_content, str):
            ...
            **输出格式示例:**
            ...
            """
            response_json = await get_ai_json_response(prompt, db,request_source="合同内容提取（文本）")
            ...
            **输出格式示例:**
            ...
            """
            response_json = await extract_info_from_multiple_images(images_bytes, prompt, db,request_source="合同内容提取（多模态）")
        else:
...
async def analyze_opponent_document(
    ...
    **输出格式 (必须严格遵守JSON格式，不要有任何额外解释):**
    ...
    """
    try:
        response_json = await get_ai_json_response(prompt, db,request_source="诉讼策略分析")
...
async def analyze_for_filing(
    ...
    **输出格式:**
    ...
    """
    try:
        response_json = await get_ai_json_response(prompt, db,request_source="立案分析")
...
async def analyze_for_judge(
    ...
    **输出格式 (必须严格遵守JSON):**
    ...
    """
    try:
        response_json = await get_ai_json_response(prompt, db,request_source="案件分析报告")
...
async def quick_scan_for_risks(
    ...
    **输出格式 (必须严格遵守JSON):**
    ...
    try:
        ...
        response = await get_ai_json_response(prompt, db, model_type="fast",request_source="快速风险分析")
...
async def autocomplete_text(
    ...
    **输出格式 (必须严格遵守JSON):**
    ...
    try:
        ...
        response = await get_ai_json_response(prompt, db, model_type="fast",request_source="快速补全")

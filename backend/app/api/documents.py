...
from ..templates.application_template import APPLICATION_TEMPLATE
        ...
        if doc_type == "答辩状":
            if settings.is_court_mode:
                ...
                if not (case_number_req and case_number_req.strip()):
                    raise HTTPException(
                        status_code=400, detail="法院模式下，填写答辩状必须提供有效的案号。")

                defendant_code = form_data.get(
                    "basicInfo", {}).get("defendantCode")
                if not defendant_code:
                    raise HTTPException(
                        status_code=400, detail="法院模式下，填写答辩状需要提供被告验证码。")
    ...
    try:
        if file:
            ...
            if isinstance(extracted_content, str):
                ...
                response_json = await ai_service.get_ai_json_response(prompt, db,request_source="提取信息(文本)")
            else:  # 是图片列表
                prompt = base_prompt + "\n(请分析以下连续的文档图片)\n---"
                response_json = await ai_service.extract_info_from_multiple_images(extracted_content, prompt, db,request_source="提取信息(多模态)")
        else:  # 只提供了文本
...
async def generate_evidence_checklist(
    ...
    """
    try:
        ai_response_json = await ai_service.get_ai_json_response(prompt, db,request_source="生成证据目录")
...
class UnsavedDocRequest(BaseModel):
    final_data: Dict[str, Any]
    doc_title: str


@router.post("/generate-unsaved")
async def generate_unsaved_document(request: UnsavedDocRequest):
    """
    接收格式化好的数据和模板名，直接生成DOCX文件并返回，不涉及数据库。
    """
    template_name = '新文档模版'
    print(request.final_data)
    try:
        template_name = APPLICATION_TEMPLATE[request.doc_title]
        output_path = f"{request.doc_title}_{int(time.time())}.docx"
        file_path = docx_service.create_docx_from_context(
            context=request.final_data["final"],
            template_name=template_name,
            output_path=output_path
        )

        encoded_filename = quote(output_path)
        headers = {
            "Content-Disposition": f"attachment; filename*=UTF-8''{encoded_filename}"
        }
        return FileResponse(
        ...
        raise HTTPException(
            status_code=404, detail=f"模板文件 '{template_name}.docx' 未找到: {e}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"生成DOCX文件时出错: {e}")

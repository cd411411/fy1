...
import { fetchAIStream } from '../api/aiApi';
...
export const AIChatbotPanel: React.FC = () => {
    const systemPrompt = useMemo<ChatMessage>(() => ({
        role: 'system',
        content: '你是要素式诉状智能生成平台的辅助AI助理，你的任务是解答用户在使用该平台中遇到的法律问题，例如：法律名词解释、填写建议、把文段优化得更符合法律用语等。除用户问你身份外，如用户提出的问题与法律或填写诉状无关，请拒绝作答，并礼貌地告知用户。你和本平台的作者都是李伯阳律师。'
    }), []);
    ...
    const handleSendMessage = useCallback(async () => {
        ...
        const historyForApi = [systemPrompt, ...messages, newUserMessage];
    ...
    }, [inputValue, isLoading, messages, systemPrompt]);
                    ...
                    {messages.map((msg, index) => (
                        <div key={index} className={`chat ${msg.role === 'user' ? 'chat-end' : 'chat-start'}`}>
                            <div className={`chat-bubble break-words ${
                                msg.role === 'user' ? 'chat-bubble-primary text-primary-content' : 'chat-bubble-secondary text-secondary-content'
                            }`}>

...
interface Props {
  ...
  onPinRecommended: (templateId: string) => void;
}

export const AIRecommender: React.FC<Props> = ({ templates, onPinRecommended }) => {
  ...
  >({
    ...
    onSuccess: (data) => {
      if (data.recommended_template_id && data.recommended_template_id !== 'null') {
          ...
          // (核心修改) 调用新的回调函数来执行置顶操作
          onPinRecommended(data.recommended_template_id);
      } else {
          toast.error("AI未能找到合适的模板，请尝试更详细地描述您的案情。");
      }
    },
    onError: (error) => {
      toast.error(`AI推荐失败: ${error.message}`);
    }
  });

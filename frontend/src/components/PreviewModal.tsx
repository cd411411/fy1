...
export const PreviewModal: React.FC<PreviewModalProps> = ({ isOpen, onClose, onConfirmWithOptions, isSubmitting, previewData, title }) => {
  ...
  const isApplicationForm = title.includes("申请书");
  return (
          ...
          <div className="space-y-8">
            ...
            {!isApplicationForm &&
              ...
              (<div className="border-b border-base-300 pb-4">
                <h3 className="text-xl font-bold mb-3">基本信息</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-base">
                  <div>
                    <span className="font-medium">案件类型：</span>
                    <span>{previewData.case_type || '未填写'}</span>
                  </div>
                  <div>
                    <span className="font-medium">案件编号：</span>
                    <span>{previewData.case_number || '未填写'}</span>
                  </div>
                </div>
              </div>
              )}
            ...
            {isApplicationForm && previewData.sections && (
              previewData.sections.map((section: { title: string, items: QuestionListItem[] }) => (
                <PreviewTableSection key={section.title} title={section.title} items={section.items} />
              ))
            )}

...
const AdminSidebar: React.FC = () => {
    ...
    const [theme, setTheme] = useState(() => localStorage.getItem("daisyui-theme") || "system");
    ...
    return (
        <aside className="w-64 bg-base-200 flex flex-col h-full">
            <div className="p-4">
                <h1 className="text-2xl font-bold text-primary">要素式诉辩状</h1>
                <h2 className="text-lg font-semibold">管理后台</h2>
            </div>
            <ul className="menu p-4 flex-grow">
                <li><NavLink to="/admin/dashboard" className={({ isActive }) => isActive ? "active" : ""}>仪表盘</NavLink></li>
                <li><NavLink to="/admin/pending-cases" className={({ isActive }) => isActive ? "active" : ""}>待立案审查</NavLink></li>
                <li><NavLink to="/admin/all-cases" className={({ isActive }) => isActive ? "active" : ""}>全部案件管理</NavLink></li>
                <li><NavLink to="/admin/ai-models" className={({ isActive }) => isActive ? "active" : ""}>AI模型管理</NavLink></li>
                <li><NavLink to="/admin/feature-settings" className={({ isActive }) => isActive ? "active" : ""}>功能设置</NavLink></li>
            </ul>
            <div className="p-4 border-t border-base-300 space-y-2">
                <div className="dropdown dropdown-top w-full">
                    ...
                    <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-full">
                        {themes.map(t => (
                            <li key={t.value}>
                                <a onClick={() => setTheme(t.value)} className={theme === t.value ? 'active' : ''}>
                                    {t.icon} {t.name}
                                </a>
                            </li>
                        ))}
...
export const AdminLayout: React.FC = () => {
    ...
    return (
        <div className="flex flex-col h-screen bg-base-100">
            ...
            <div className="flex-1 drawer lg:drawer-open overflow-y-hidden">

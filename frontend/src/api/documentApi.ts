 ...
 * @param documentTypeName 文档类型
 * @param payload 负载数据
 * @param caseTitle 案件标题
 * @returns 包含响应头的对象，用于提取案号和验证码
 */
export const generateAndDownloadDocx = async (
  documentTypeName: "起诉状" | "答辩状" | "申请书",
  payload: any,
  caseTitle: string
): Promise<{ headers: any }> => {
  try {
    ...
    if (documentTypeName != "申请书") {
      ...
      response = await apiClient.post(
        `/api/documents/generate-unsaved`,
        requestBody,
        {
          responseType: "blob",
        }
